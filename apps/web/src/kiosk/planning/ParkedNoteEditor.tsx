 ...
 * they are not the same shape — the Horizon board lists what this session parked, the
 * shell's gold box raises what was addressed to the step you are standing on — but the
 * EDIT is the same edit, so it is one component rather than two that drift. Only the
 * hosting row differs.
 *
 * THE TAG ROW IS THE HOST'S TO OFFER. The Horizon board hands it the same tags the park
 * bar offered (correcting a note there offers exactly what parking it offered); the gold
 * box hands it every step this household runs, because a note that has already LANDED
 * somewhere must be re-addressable to anywhere, including a step the bar would not have
 * suggested. Either way the note's CURRENT tag is always a chip — see `chips` — so a note
 * routed to Calendar by step 1 can be edited on a surface whose list omits Calendar
 * without the row quietly losing its own tag.
 *
 * "No tag" is the ABSENCE of a tag and so is never a row the server sends: it is the chip
 * that submits `stepKey: null`.
 */
...
export interface ParkedNoteEditorProps {
  ...
  sessionId?: string
  /** A write is in flight elsewhere — the whole editor disables with it. */
  busy?: boolean
  onCancel: () => void
...
export function ParkedNoteEditor({
  ...
  // The cursor starts in the field: you came here to change the words, and the tag chips
  // are one tab away either way.
  useEffect(() => {
    inputRef.current?.focus()
    inputRef.current?.select()
  }, [])

  // The current tag is always representable, even when this surface's list doesn't
  // contain it — a routed note can carry a tag the park bar never offers.
  ...
  function save(e: FormEvent) {
    ...
    looseEndsApi
      .update(id, {
        // Only what actually moved. Both fields are read for PRESENCE server-side, so
        // sending the tag unchanged would still rewrite the route entry for no reason.
        ...(trimmed === note.trim() ? {} : { note: trimmed }),
        ...(tag === stepKey ? {} : { stepKey: tag }),
        ...(sessionId ? { sessionId } : {}),
      })
      .then((r) => onSaved({ id: r.item.id, note: r.item.note, stepKey: r.item.stepKey }))
      // KEEP THE SENTENCE — the server's cap and its "that step isn't running" are the
      // useful half of a refusal, exactly as the park bar keeps them.
      .catch((err: unknown) =>
        setError(
