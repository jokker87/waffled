...
//
// `dueOn` is not a chore column (a one-off's day lives on its single instance), so it is
// absent from `UPDATABLE_CHORE` — and the route used to gate on that map alone, which meant
// a patch carrying ONLY `dueOn` was refused as "no updatable fields provided" even though
// the service implements it fully. That broke the Weekly Planning Tasks step's day chip,
// whose whole write is `{ dueOn }`; it went unnoticed because every other caller sends a
// whole form, so `dueOn` always had a companion field.
//
// One exported list so the gate and the implementation cannot drift again: a field the
// service handles has to appear here to be reachable, and this is the only place the route
// looks.
...
/// The chore row as it stands, for the paths where there is nothing to write to it but the
/// caller still needs the row back (a dueOn-only patch moves an instance, not a column).
...
export async function updateChore(
  ...
  try {
    ...
    if (scope === 'following') {
      ...
      // A dueOn-ONLY patch touches no chore COLUMN — a one-off's day lives on its single
      // instance — so `updateTemplate` returns null for it. That is not "nothing to
      // update": the dueOn handling below is the update. Only a patch with neither a
      // template field NOR a dueOn has nothing to do.
