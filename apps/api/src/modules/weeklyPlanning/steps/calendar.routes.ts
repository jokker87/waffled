...
// it reads `GET /api/events?from&to` over weekStart…weekStart+6 and adds through the
// app's own event modal, which writes with `POST /api/events`. Both already exist, are
// already tenant-scoped, and are already what every other calendar surface uses — a
// `/api/weekly-planning/calendar` mirror of them would be a second door onto the same
