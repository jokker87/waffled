...
// Step 5 · Connection — the reads this step needs over event participants.
//
...
// sibling `connection.ts`; integration tests in
// apps/api/test/weekly-planning-connection.integration.test.ts.
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const { tenantRoute, adminRoute } = moduleRoutes('weeklyPlanning')

export function registerConnectionStepRoutes(_api: Api): void {
  // e.g. _api.get('/api/weekly-planning/connection', tenantRoute(async (tenant) => { … }))
}
