...
describe('chore management (edit/delete)', () => {
  ...
  // A dueOn-ONLY patch is the Weekly Planning Tasks step's whole write ({ dueOn }), and the
  // route used to refuse it as "no updatable fields provided" because dueOn is not a chore
  // COLUMN. Every other caller sends a whole form, so dueOn always had a companion and
  // nothing noticed. This is the single-field case, pinned.
