...
| **Meal Builder** — build one meal out of several recipes (a "plate") | ✅ | ✅ | ✅ | ✅ Done — name it, add recipes under **Main / Sides / Dessert**, set meal-level servings, assign **a cook per dish**. Saved meals are first-class in the recipe library (a `Meal · N` badge, a 🍽️ **Meals** filter, search spans the meal's name *and* its dish titles); adding a saved meal to a plate flattens it into its dishes. Adding a dish is a tap through the recipe/meal picker on every platform; a dish already on the plate can be **dragged between roles** (on iPhone/iPad via a press-and-hold over one flat run of rows — a `List` reorders only within a section and refuses `.dropDestination`). **Web also builds a plate from inside the recipe picker** — ＋ New meal opens this same builder over the picker and drops the finished plate on the night you were filling, saving it to the library on the way past; iOS still reaches the builder only from the Meals screen |
...
| **Weekly Planning** module — a guided session that decides the week ahead in ten steps (loose ends · calendar · horizon scan · family night · connection · goals · meals · tasks · kids · recap). Settings → Modules sets the session's day/time and turns individual steps off; the **Planning** page holds the lobby, the step chrome (counter → agenda sheet, the one question each step asks, skip/affirm footer) and the saved record of what the week decided. Defaults to the week ahead, with a stepper for planning any later week (each week gets its own session and record); every step has its own URL, so leaving and returning — or a refresh, or the back button — resumes exactly where you were, with **Leave for now** in the session header (and in the agenda sheet), which returns you to the lobby holding the part-planned session — resume it, or step to another week — rather than resuming it automatically on the next visit, and **Start this week over** in the sheet (starting over discards the session record only — what it already decided stays in the calendar/chores/goals) | ✅ | 🟡 | 🟡 | 🟡 Web complete, iOS parity outstanding — the **shell** (module toggle + settings panel, the `/planning` lobby, the session chrome, the saved record, and the parked-note handoff that opens each step with whatever was tagged for it — the step's own verb where it has one (**Make a task**, **Make an event**, opening that step's existing composer with the note's words in it), plus Handled / Drop it / leave it parked) plus **all 10 steps**: **Loose ends** (what's still open + parked notes, each routed to the step that will handle it — routing writes nothing to your modules), **Calendar** (the real week, add-in-place on a tapped day), **Goals** (a tab per goal list, focus via the existing featured flag, pins you made by hand are never cleared), **Meals** (dinners in the same seven columns, fill-only-the-empties with undo, a night fillable with a recipe **or a saved plate** — built in place with the real Meal Builder if it doesn't exist yet — and the shopping trip assignable as a real chore), **Tasks** (per-person columns, an up-for-grabs strip, drag or tap to hand a chore over, and the existing chore modal for both adding — Who prefilled — and editing, reached from a task's name or the day beside it). **Horizon scan** (the shipped month view plus one bar that parks a note — never a calendar entry — tagged for one of the steps still *ahead* of you, which then opens with it; an untagged note stays on the board for the recap and next week's loose ends, and the bar says so), **Family night** (the rotation's suggestion per part, pinnable for this week only, a line per part for what it actually is, an optional theme, getting *this* week onto the calendar as a new event or by pointing at one already on the week, and calling the week off without touching the standing recurring event), **Connection** (pairings read out of `event_participants`, time you are *already* spending together counted rather than replaced — including **Link a time**, which lists every event on the week with both of you on it, whoever else is there too, and lets you say that one *is* your time without editing anybody's event — and any pairing makeable from scratch, every one landing as an ordinary calendar event), **Kids** (a card per child with their week, stars, a focus drawn from their own goals and overdue chores, and something to look forward to picked off their own calendar, then read back to them — the focus also surfaces on that child's Family profile as "this week's one thing", read from the session rather than copied). **Recap** (the week read back one last time, with what the session changed grouped by the module each decision lives in — every line a pointer to live data rather than a copy — plus a last call on notes nobody tagged and a column for what was left alone on purpose). Remaining for the module: iOS parity. A step whose module is off is skipped automatically and never counted, so the counter reads "3 of 7" for a household that doesn't use meals |
